...
It is always a good idea to use informative titles and axis labels on your plots. Unfortunately, the default plots produced by R try and *guess* labels from the data, which may not be that useful. 
...
```{r}
data <- read.csv("PrimerExamples/Paired two-sample t-test.csv")
plot(data[,1])
points(data[,2])
```
...
```{r}
plot(data[,1],col="steelblue")
points(data[,2],col="orange")
```

Or change the [plotting characters](#plotting-characters-and-their-values)

```{r}
plot(data[,1],col="steelblue",pch=16)
points(data[,2],col="orange",pch=17)
```
...
```{r}
data <- read.csv("PrimerExamples/One-sample t-test.csv")
plot(data[,2])
text(data[,2],labels=LETTERS[1:10])
```
...
In order to achieve this we can use a matrix object.  The code below generates a small matrix.
...
Extracting subsets from a list is slightly more subtle than for other object types.  Where as before we used a single set of square brackets `[]`, lists makes use of both this and a double set of brackets `[[]]`.

When using a single set of brackets on a list, R will return another list.  This makes sense if you're interested in getting a few entries but can be a little unintuative if you're only interested in a single element.

If you wish to extract a single element and have it returned 
