...
The boxplot is a convenient way of comparing the distributions of numeric data. If given a numeric matrix, or data frame, it will display the summary statistics of each column in a separate box.
...
The `legend` function can be used to add a legend to an existing plot. In the following example, we generate a random matrix and plot the second, third and fourth columns against the first. We use red circles, blue squares and orange triangles respectively to represent these variabes. We can define the x and y coordinates of where the legend will appear. Alternatively, the strings `topleft`, `topright`, `bottomleft` and `bottomright` are conveniant shortcuts. The names to be used in the legend are defined by the `legend` argument, and we can also define the relevant colours and plotting characters.


```{r}
randMat <- matrix(rnorm(100),ncol=10)
plot(randMat[,1],randMat[,2],ylim=c(-3,3),pch=16,col="red",xlab="Column1",ylab="y")
points(randMat[,1],randMat[,3], pch=15,col="steelblue")
points(randMat[,1],randMat[,4], pch=17,col="chocolate3")

legend("topright", legend=c("Column 2", "Column 3","Column 4"), col = c("red", "steelblue", "chocolate3"), pch=c(16,15,17))
```
...
legend("topleft", legend=c("Column 2", "Column 3","Column 4"), col = c("red", "steelblue", "chocolate3"), pch=c(16,15,17))
```

A legend can be added to all types of plots; as shown here for a [boxplot](#a-boxplot).
