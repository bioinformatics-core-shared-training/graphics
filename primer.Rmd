...
R can produce scatter plots of two vectors. The vector supplied as the first argument will be plotted on the *x* axis and the second vector will be plotted on the *y* axis. Both vectors **must** have the same length. The axis limits and limits are inferred from the data. 
...
```{r}
randmat <- matrix(rnorm(100,10,5),ncol=10)
boxplot(randmat)
```

```{r}
expression1 <- rnorm(5,4,1)
expression2 <- rnorm(5,6,1)

expression.all <- c(expression1, expression2)
group <- c(rep("A",5),rep("B",5))

boxplot(expression.all~group)
```
