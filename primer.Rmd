...
The function `strsplit()` allows you to break a string into smaller parts, based on a specifc criteria.  In the example below we break the name of first entry in the mtcars data where there is a space.

```{r}
strsplit(x = rownames(mtcars)[1], split = " ")
```


###Extracting substrings

If we only want to retain a portion of a string we can use the `substring` function.  In addition to the strings we want to process this takes two arguments: `start` and `stop` which are integers specifying the first and last characters we want to retain.  In the example below we retain the first 4 characters.

```{r}
substr(rownames(mtcars), start = 1, stop = 4)
```
...
The default behavious of `paste()` is to place spaces between the string.  If we want to alter this we need to specify the `sep` argument.  Here we combine the months of the year with the number of characters in each word.
