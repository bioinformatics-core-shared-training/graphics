...
The data that you need for this practical are available at ......... Please download this zip file and extract to your computer. You should then change your working directory in RStudio to point to the directory that you have just created. Session $\rightarrow$ Change Working directory.
...
Some of the observations in the data frame are clearly predictions based on previous years. We can make predictions after first fitting a linear model to the data


<<>>=
traindata <- life[110:160,]
mod <- lm(traindata[,2]~traindata[,1])
mod
newy <- predict(mod, newdata = data.frame(x=2001:2050))

plot(life[161:211,2],newy)
@
...
\exercise Similarly, verify that each row in the feature information corresponds to the first column of the expression matrix. You may wish to use the \Rfunction{all}
...
\exercise Find the probe ID that corresponds to the gene symbol "ESR1". First use \Rfunction{grep} to search for ESR1 in the column that gives gene names. Then, find the probe ID that represents this gene.
