...
\exercise Use the \Rfunction{extractSurvivalEvent} function to get the Event vector from the data matrix. There is only one argument to this function; a data matrix that contains columns that we want to extract event information from.
...
\exercise Repeat the steps for the example dataset \Rcode{data/Three groups.txt}
