...
- Each column is a variable representing a different group.
- We call this wide data
...
Something about Mike's functionality....
```{r echo=FALSE}
...
- We use *formula* syntax with the `~` symbol. e.g. \Rcode{y ~ x}
...
- Plot avearges with error bars

##Example

Subset and create averages
```{r}
ctrl <- data[,2:4]
means <- rowMeans(ctrl,na.rm=TRUE)
```
calculate standard deviations
```{r eval=FALSE}
vars <- c(sd(ctrl[1,]), sd(ctrl[2,]),...)
```

Repeat for treatment group
